"""
Store Django App Configuration
Marketplace integration and order management
...
class StoreConfig(AppConfig):
    ...
    name = 'modules.store.backend'
    label = 'store'
    verbose_name = 'Store - Marketplace & Orders'
    ...
    def _add_sdk_to_path(self):
        ...
        try:
            # Get project root (from modules/store/backend -> go up 3 levels)
    ...
    def _initialize_module(self):
        ...
        try:
            ...
            # Initialize module
            self.unibos_module = UnibosModule('store')

            # Log initialization
            import logging
            logger = logging.getLogger(__name__)
            logger.info(f"✓ Initialized UNIBOS module: store v{self.unibos_module.manifest.get('version')}")
