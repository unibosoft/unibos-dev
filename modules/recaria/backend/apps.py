"""
Recaria Django App Configuration
Medieval-themed consciousness exploration MMORPG
...
class RecariaConfig(AppConfig):
    default_auto_field = 'django.db.models.BigAutoField'
    name = 'modules.recaria.backend'
    label = 'recaria'
    verbose_name = 'Recaria - Consciousness Exploration'
    ...
    def _add_sdk_to_path(self):
        ...
        try:
            # Get project root (from modules/recaria/backend -> go up 3 levels)
    ...
    def _initialize_module(self):
        ...
        try:
            ...
            # Initialize module
            self.unibos_module = UnibosModule('recaria')

            # Log initialization
            import logging
            logger = logging.getLogger(__name__)
            logger.info(f"✓ Initialized UNIBOS module: recaria v{self.unibos_module.manifest.get('version')}")

            # Ensure storage paths exist
            self.unibos_module.get_storage_path('game_data/')
            self.unibos_module.get_storage_path('character_backups/')
