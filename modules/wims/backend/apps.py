"""WIMS Django App Configuration - UNIBOS Module Integration"""
...
class WimsConfig(AppConfig):
    ...
    name = 'modules.wims.backend'
    label = 'wims'
    verbose_name = 'WIMS - Where Is My Stuff'
    ...
    def _initialize_module(self):
        try:
            ...
            self.unibos_module = UnibosModule('wims')
            import logging
            logging.getLogger(__name__).info(f"✓ Initialized UNIBOS module: wims v{self.unibos_module.manifest.get('version')}")
            self.unibos_module.get_storage_path('items/')
