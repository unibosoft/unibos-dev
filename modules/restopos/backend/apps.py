"""
RestoPOS Django App Configuration
...
class RestoposConfig(AppConfig):
    ...
    name = 'modules.restopos.backend'
    label = 'restopos'
    verbose_name = 'RestoPOS - Restaurant POS'
    ...
    def _initialize_module(self):
        ...
        try:
            ...
            self.unibos_module = UnibosModule('restopos')
            import logging
            logger = logging.getLogger(__name__)
            logger.info(f"✓ Initialized UNIBOS module: restopos v{self.unibos_module.manifest.get('version')}")
            self.unibos_module.get_storage_path('receipts/')
            self.unibos_module.get_storage_path('menu_images/')
