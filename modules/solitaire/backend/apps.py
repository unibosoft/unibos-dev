"""Solitaire Django App Configuration - UNIBOS Module Integration"""
...
class SolitaireConfig(AppConfig):
    ...
    name = 'modules.solitaire.backend'
    label = 'solitaire'
    verbose_name = 'Solitaire Card Game'
    ...
    def _initialize_module(self):
        try:
            ...
            self.unibos_module = UnibosModule('solitaire')
            import logging
            logging.getLogger(__name__).info(f"✓ Initialized UNIBOS module: solitaire v{self.unibos_module.manifest.get('version')}")
            self.unibos_module.get_storage_path('sessions/')
