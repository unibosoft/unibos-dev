"""
Music Django App Configuration
Music collection management with Spotify integration
...
class MusicConfig(AppConfig):
    ...
    name = 'modules.music.backend'
    label = 'music'
    verbose_name = 'Music Collection'
    ...
    def _initialize_module(self):
        ...
        try:
            ...
            self.unibos_module = UnibosModule('music')
            import logging
            logger = logging.getLogger(__name__)
            logger.info(f"✓ Initialized UNIBOS module: music v{self.unibos_module.manifest.get('version')}")
            self.unibos_module.get_storage_path('album_art/')
            self.unibos_module.get_storage_path('artist_images/')
