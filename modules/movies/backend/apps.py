"""
Movies Django App Configuration
Movie and TV series collection management
...
class MoviesConfig(AppConfig):
    ...
    name = 'modules.movies.backend'
    label = 'movies'
    verbose_name = 'Movies & TV Series'
    ...
    def _initialize_module(self):
        ...
        try:
            ...
            self.unibos_module = UnibosModule('movies')
            import logging
            logger = logging.getLogger(__name__)
            logger.info(f"✓ Initialized UNIBOS module: movies v{self.unibos_module.manifest.get('version')}")
            self.unibos_module.get_storage_path('posters/')
            self.unibos_module.get_storage_path('backdrops/')
